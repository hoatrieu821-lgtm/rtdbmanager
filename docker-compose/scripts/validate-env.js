...
function isValidHttpUrl(v) {
  try {
    const u = new URL(v);
    return u.protocol === "http:" || u.protocol === "https:" ? null : "must use http or https";
  } catch {
    return "must be a valid URL";
  }
}

function isValidFirebaseRtdbUrl(v) {
  try {
    const u = new URL(v);
    ...
    if (!isFirebaseHost) return "must be a Firebase Realtime Database URL";
    return null;
  } catch {
    return "must be a valid URL";
  }
...
function minLength(n) {
  return (v) => (String(v).length >= n ? null : `must be at least ${n} characters`);
}

function isAllowedEmailList(v) {
  if (v === "*") return null;
  const emails = v.split(",").map((s) => s.trim()).filter(Boolean);
  if (!emails.length) return "must include at least one email or *";
  return emails.every((email) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
    ? null
    : "must be comma-separated email addresses or *";
}
...
checkRequired("MASTER_FIREBASE_PROJECT_ID", "RTDB Manager master Firebase project id", (v) =>
  /^[a-z0-9][a-z0-9-]*$/.test(v) ? null : "only lowercase letters, numbers, hyphen"
);
checkRequired("MASTER_FIREBASE_DATABASE_URL", "RTDB Manager master Realtime Database URL", isValidFirebaseRtdbUrl);
checkRequired("MASTER_FIREBASE_CLIENT_EMAIL", "RTDB Manager Firebase service-account email", (v) =>
  /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v) ? null : "invalid email"
);
checkRequired("MASTER_FIREBASE_PRIVATE_KEY", "RTDB Manager Firebase service-account private key", (v) => {
  const raw = v.replace(/\\n/g, "\n");
  return raw.includes("-----BEGIN PRIVATE KEY-----") && raw.includes("-----END PRIVATE KEY-----")
    ? null
    : "must contain a PEM private key";
});
checkRequired("SESSION_SECRET", "RTDB Manager session signing secret", minLength(32));
checkRequired("CRYPTO_KEY", "RTDB Manager credential encryption secret", minLength(32));
checkRequired("FIREBASE_API_KEY", "Firebase Web API key for browser login", minLength(10));
checkRequired("API_SECRET_KEY", "X-API-Key secret for /api/v1", minLength(16));
checkRequired("ALLOWED_EMAILS", "Google sign-in allowlist", isAllowedEmailList);
...
checkOptional("APP_BASE_URL", "public browser URL for RTDB Manager", isValidHttpUrl);
checkOptional("LOG_LEVEL", "pino log level");
checkOptional("FIREBASE_AUTH_DOMAIN", "Firebase Auth domain");
checkOptional("FIREBASE_APP_ID", "Firebase web app id");
checkOptional("FIREBASE_MESSAGING_SENDER_ID", "Firebase messaging sender id");
checkOptional("FIREBASE_STORAGE_BUCKET", "Firebase storage bucket");
checkOptional("MASTER_BACKUP_PASSWORD", "master database dump/restore password", minLength(32));
